...
const stripPrefix = (msg: string) => msg.replace(/^Value error,\s*/i, '')

export const getApiError = (err: any, fallback = 'Something went wrong'): string => {
  const detail = err?.response?.data?.detail
  if (Array.isArray(detail)) {
    return detail.map((d: any) => stripPrefix(d.msg ?? String(d))).join('; ')
  }
  if (typeof detail === 'string') return stripPrefix(detail)
